...
function parallel_custom_header_setup() {
    $args = array(
        'default-image'      => esc_url(get_template_directory_uri()) . '/images/bg-demo.jpg',
        'header-text'        => false,
        'width'              => 1920,
        'height'             => 900,
        'flex-width'         => true,
...
function parallel_customizer_css() {
    ...
    <style type="text/css">
        ...
        <?php else : ?>
        .hero.default {background: url(<?php echo esc_url(get_template_directory_uri()); ?>/images/bg-demo.jpg) no-repeat center top; background-size: cover;}
        <?php endif; ?>
    </style>
    <?php
}
...
function parallel_customize_preview_js() {
	wp_enqueue_script( 'parallel_customizer', esc_url(get_template_directory_uri()) . '/js/customizer.js', array( 'customize-preview' ), '20130508', true );
}
