...
class Parallel_Welcome {
	...
	public function __construct() {
		...
		add_action( 'parallel_welcome', array( $this, 'parallel_welcome_import_demo' ), 		    30 );
		add_action( 'parallel_welcome', array( $this, 'parallel_welcome_child_themes' ), 		    40 );

		/* Dismissable notice */
		add_action('admin_init', array($this,'dismiss_welcome'), 1);

		/* Toolbar upgrade link */
		add_action( 'admin_bar_menu', array($this, 'parallel_lite_upgrade_toolbar'), 999 );
	}
	 ...
	 */
	function parallel_lite_upgrade_toolbar( $wp_admin_bar ) {

		$args = array(
			'id'    => 'themely-upgrade-parallel',
			'title' => esc_html__('Upgrade to Parallel Pro ➡', 'parallel'),
			'href'  => 'https://www.themely.com/themes/parallel/',
	...
	public function parallel_welcome_screen() {
		...
		<div class="wrap about-wrap theme-welcome">
            ...
            <h1><?php esc_html_e('Welcome to Parallel', 'parallel'); ?> <span><?php esc_html_e('VERSION 1.4.8', 'parallel'); ?></span></h1>
			...
			<ul class="nav-tab-wrapper" role="tablist">
	            ...
	            <li role="presentation" class="nav-tab"><a href="#theme_support" aria-controls="theme_support" role="tab" data-toggle="tab"><?php esc_html_e( 'Help & Tutorials','parallel'); ?></a></li>
